...
% the JupyterNotebook "downloadBGCandPHYSfromCMEMS.ipynb".                %
%                                                                         %
...
%            BGC - light attenuation coefficient (kd): m-1                % 
...
% As in the JupyterNotebook "downloadBGCandPHYSfromCMEMS.ipynb"
...
% Loop over files
for iDataset = 1:length(filenameInput)

    fileName = filenameInput{iDataset};
    ...
    if (memoryRequiredMB < 16000) 
        ...
        Dclim = NaN(newDimCounts,'double'); 
        if ~isempty(iDepth)
            Dtmp = ncread(filePath,oceanColourVarName{1},start,count);
    ...
    % are not yet a climatology (BGC variables)
        ...
        if isempty(iDepth)
            ...
            Dclim_err = NaN([numel(newLat),numel(newLon),12],'double'); 
        else
            Dclim = NaN([numel(newLat),numel(newLon),numel(depth),12],'double'); 
            Dclim_err = NaN([numel(newLat),numel(newLon),numel(depth),12],'double'); 
        end
        ...
        for iMonth = 1:12
            ...
            if ~isempty(iDepth)
                ...
                Dtmp_err = std(D(:,:,:,idxMonth),0,4,'omitnan');
                Dperm = permute(Dtmp,[iDimLat iDimLon iDimDepth iDimTime]);
                Dperm_err = permute(Dtmp_err,[iDimLat iDimLon iDimDepth iDimTime]);
                Dclim(:,:,:,iMonth) = Dperm;
                Dclim_err(:,:,:,iMonth) = Dperm_err;
            elseif isempty(iDepth)
                Dtmp = mean(D(:,:,idxMonth),3,'omitnan');
                Dtmp_err = std(D(:,:,idxMonth),0,3,'omitnan');
                Dperm = permute(Dtmp,[iDimLat iDimLon iDimTime]);
                Dperm_err = permute(Dtmp_err,[iDimLat iDimLon iDimTime]);
                Dclim(:,:,iMonth) = Dperm;
                Dclim_err(:,:,iMonth) = Dperm_err;
            end
        end
    ...
    % Save output
    saveAndVisualiseOutput(iDataset,Dclim,Dclim_err,lat,lon,depth,...
...
function saveAndVisualiseOutput(iDataset,Dclim,Dclim_err,lat,lon,depth,...
    fullpathOutputCmemsDir,filenameOutput)
    ...
    if iDataset == 1
        ...
        npp_lat = lat;
        npp_lon = lon;
        save(fullfile(fullpathOutputCmemsDir,filenameOutput{iDataset}),...
